...
The icon system uses **25 buckets** (medium granularity). Buckets are stable, language-independent identifiers (`fruit`, `vegetable`, `dairy`, `egg`, `cheese`, `meat`, `fish`, `deli-cold-cuts`, `bread`, `grain`, `pasta`, `beverages-cold`, `beverages-hot`, `alcohol`, `frozen`, `snacks`, `sweets`, `condiments`, `staples`, `pantry`, `cleaning`, `personal-care`, `pet`, `baby`, plus `generic`). Oils and herbs/spices are folded into `condiments`; the bucket name `pantry` (formerly `pantry-canned`) covers all long-shelf-life staples including dry goods and canned items; `staples` (formerly `mercearia`) covers bulk dry basics like flour, sugar, and salt. The icon for each bucket is decided once in code; the dictionary maps terms to buckets, never directly to icons. Adding or swapping an icon library is a one-file change.

**Icon choice notes:** Material Icons Rounded lacks dedicated glyphs for some categories. `FRUIT` uses `LocalFlorist` (flower) — the closest nature-produce glyph available; `CONDIMENTS` uses `SoupKitchen` (stockpot) — suggests cooking/seasoning rather than a specific condiment shape. These are the best available options in `material-icons-extended` and should be revisited if migrating to Material Symbols (see ADR-0001).
